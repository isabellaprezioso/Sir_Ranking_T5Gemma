...
class PredictionLayer(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # subito dopo la scelta del modello
        self.model.opt = opt  # passa opt al modello
...
class LFM(nn.Module):
    ...
    def forward_oldNARRE(self, feature, user_id, item_id):
    ...
    def forward(self, feature, user_id=None, item_id=None):
        # Se siamo in modalità T5 o mancano uid/iid, salta i bias
        if hasattr(self, 'opt') and getattr(self.opt, 'use_t5', False):
            return self.fc(feature)
        elif user_id is not None and item_id is not None:
            return self.fc(feature) + self.b_users[user_id] + self.b_items[item_id]
        else:
            return self.fc(feature)
...
class FM(nn.Module):
    ...
    def forward(self, feature, uids=None, iids=None):
        fm_out = self.build_fm(feature)

        if hasattr(self, 'opt') and getattr(self.opt, 'use_t5', False):
            # Con T5 non sommare bias
            return fm_out
        elif uids is not None and iids is not None:
            return fm_out + self.b_users[uids] + self.b_items[iids]
        else:
           #  user_bias = self.b_users[uids].squeeze(1)  # Rimuove la dimensione 1
           #  item_bias = self.b_items[iids].squeeze(1)  # Rimuove la dimensione 1
           # return fm_out + user_bias + item_bias
            return fm_out
