...
"RIMANE LO STESSO PER T-5 GEMMA -> (produce solo testi puliti e mappa user/item → id)"
"BUILD DOC (unisce recensioni per ciascun utente o item *join_text ) CLEAN DOC (taglia recensioni a 500)"
" risultato restituito a BUILD DOC "
" clean_doc restituisce le liste di token troncate. "
" Queste vengono poi convertite in ID di parola (in userDoc2Index) e salvate nel file userDoc2Index.npy"
 #"user doc = "userDoc2index" "
 # "user_review2doc, item_review2doc, user_reviews_dict, item_reviews_dict )"
...
def build_doc(u_reviews_dict, i_reviews_dict):
    ...
    u_doc = clean_doc(u_reviews)  #QUI HO DOCUMENTO UNICO DELLE RECENSIONI PER UTENTE "user_doc"
    i_doc = clean_doc(i_reviews)  #QUI HO DOCUMENTO UNICO DELLE RECENSIONI PER ITEM "item_doc"
...
if __name__ == '__main__':
    ...
    with open(os.path.join(save_folder, 'user2id.json'), 'w', encoding='utf-8') as f:
        json.dump(user2id, f, ensure_ascii=False, indent=2)

    with open(os.path.join(save_folder, 'item2id.json'), 'w', encoding='utf-8') as f:
    ...
    #SALVA FILE CHE PRENDE REVIEW DATA ! -> T5 GEMMA
    ...
    vocab, user_review2doc, item_review2doc, user_reviews_dict, item_reviews_dict = build_doc(user_reviews_dict,
                                                                                              item_reviews_dict)
    #user_review2doc -> ogni elemento della lista è una lista di toke che rappresenta recensioni unite e troncate a 500 token
    #item_review2doc
    #user_reviews_dict -> mappano id utente ad una lista di stringhe
    #item_reviews_dict
